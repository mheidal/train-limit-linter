...
---@field train_schedule_groups table<string, TrainScheduleGroup> string is the canonical schedule name (name of all non-excluded stations in the schedule)

---@class TrainScheduleGroup
---@field trains LuaTrain[]
---@field canonical_schedule TrainScheduleRecord[]
---@field all_schedules TrainScheduleRecord[][]
...
function Exports.new_get_train_groups(excluded_keywords, hidden_keywords)
...
function Exports.get_train_schedule_groups_by_surface(excluded_keywords)
    ...
    for _, surface in pairs(game.surfaces) do
        ...
        for _, train in pairs(surface.get_trains()) do
            ...
            if schedule then
                added_schedule = true
                local key = utils.train_records_to_key(schedule)
...
---@return ScheduleTableData: info about train stops
function Exports.new_get_train_stop_data(train_group, surface, rails_under_trains_without_schedules)
    ---@type ScheduleTableData
    local ret = {
        limit=0,
        not_set=false,
